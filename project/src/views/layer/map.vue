    ...
    <div class="main-map">
      <div class="row" v-for="(row, rowIndex) in mRowList" :key="rowIndex">
        <div
          class="cell"
          :style="mStyle"
          v-for="(row, columnIndex) in mColumnList"
          :key="columnIndex"
        >
          <div
            class="cell-content"
            @click="onClickBlock(rowIndex, columnIndex)"
            @contextmenu.prevent="onRightClickBlock(rowIndex, columnIndex)"
          >
            <div class="float-block" v-if="!isBlockOpen(rowIndex, columnIndex)">
              <div
                class="red-flag-s"
                v-if="isBlockFlag(rowIndex, columnIndex)"
              ></div>
            </div>
            <span
              class="text-num"
              :style="getBombNumberStyle(rowIndex, columnIndex)"
              v-if="
                isBlockOpen(rowIndex, columnIndex) &&
                getBlockInfo(rowIndex, columnIndex) > 0
              "
            >
...
export default {
  ...
  computed: {
    getFlagSelectStyle() {
      var style = ``;
      if (this.mSelectFlag) {
        style = `border-color: rgb(128, 128, 128) rgb(255, 255, 255) rgb(255, 255, 255) rgb(128, 128, 128); background:rgb(98,183,135)`;
      }
      return style;
    },
